...
class Client:
  ...
  def login(self, username, password, token='', sBox=True):
    if sBox == True:
      self.serverUrl = "https://test.salesforce.com/services/Soap/u/25.0"
    else:
      self.serverUrl = "https://login.salesforce.com/services/Soap/u/25.0"

    lr = LoginRequest(self.serverUrl, username, password+token).post()
